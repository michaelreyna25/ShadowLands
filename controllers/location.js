...
const cap = captured.get({plain : true})
  ...
  switch (progress) {
    case 0:
      ...
      const { intro, wilds, bosss } = introData.get({ plain: true });

      //randomly select index of wild monster and boss
      const opponent = Math.floor(Math.random() * wilds.length);
      const beast = Math.floor(Math.random() * bosss.length);

      //update table attributes
      // await Location.update(
      //   { opponent_id: wilds[opponent].id, beast_id: Bosss[beast].id },
      //   { where: { id: p_id } }
      // );
      ...
      console.log("shadowbeat", Bosss);

      html = "intro";
      data = {
        intro,
        wild_id: wilds[opponent].id,
        beast_id: bosss[beast].id,
      };
      //console.log({intro, wild_id: wilds[opponent].id, beast_id: Bosss[beast].id})
      break;
